...
best_model_name = str(eval_df.loc[best_model_idx, 'Model'])  # type: ignore
best_r2 = float(eval_df.loc[best_model_idx, 'Test R²'])  # type: ignore
best_mae = float(eval_df.loc[best_model_idx, 'Test MAE'])  # type: ignore
...
plt.plot(test_data['date'].to_numpy(), y_test.to_numpy(), '-', color='#1e293b', linewidth=3, label='Actual Sales', zorder=3)
...
r2_values = eval_df['Test R²'].to_numpy(dtype=float)
bars = plt.bar(eval_df['Model'].to_numpy(dtype=str), r2_values, color=LINE_COLORS, alpha=0.85, edgecolor='none', width=0.6)
...
upper_bound = forecast_df['forecasted_sales'].to_numpy(dtype=float) + best_mae
lower_bound = forecast_df['forecasted_sales'].to_numpy(dtype=float) - best_mae
ax.fill_between(forecast_df['date'].to_numpy(), lower_bound, upper_bound, alpha=0.15, color=PRIMARY_GREEN)
...
print(f"\n2. MODEL PERFORMANCE")
print(f"   • Selected Model: {str(best_model_name)}")
print(f"   • Model Accuracy (R² Score): {best_r2:.2%}")
print(f"   • Average Prediction Error: ±${best_mae:,.2f}")
print(f"   • Prediction Confidence: {(1 - (best_mae/float(historical_avg)))*100:.1f}%")
...
SELECTED MODEL:
  ...
  • Prediction Confidence: {(1 - (best_mae/float(historical_avg)))*100:.1f}%
