...
@app.route('/api/models') # Changed to api/models for better structure
...
@app.route('/models/')
def models_index():
    """List models or serve linear regression if available (legacy support)"""
    try:
...
def upload_file():
    ...
    try:
        ...
        best_model_idx = int(eval_df['Test R²'].idxmax())
        best_model_row = eval_df.iloc[best_model_idx]
